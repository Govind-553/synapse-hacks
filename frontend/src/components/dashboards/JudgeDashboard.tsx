...
const JudgeDashboard: React.FC<JudgeDashboardProps> = ({ user, token }) => {
  ...
  const staticAssignedEvents: AssignedEvent[] = [
    {
      id: 1,
  ...
  const staticSubmissions: Submission[] = [
    {
      id: 1,
      teamName: "Team Alpha",
  ...
  useEffect(() => {
    if (useStaticData) {
      setAssignedEvents(staticAssignedEvents);
      setSubmissionsToReview(staticSubmissions);
    ...
    const fetchAssignedEvents = async () => {
      ...
      try {
        const response = await fetch(`${API_BASE_URL}/judges/${user.id}/events`, {
    ...
    const fetchSubmissionsToReview = async (eventId) => {
      setLoadingState((prev) => ({ ...prev, submissions: true }));
      try {
        const response = await fetch(`${API_BASE_URL}/judges/events/${eventId}/submissions?judgeId=${user.id}`, {
          headers: { "Authorization": `Bearer ${token}` },
        });
        ...
        setLoadingState((prev) => ({ ...prev, submissions: false }));
      }
    };

    const fetchLeaderboard = async (eventId) => {
      setLoadingState((prev) => ({ ...prev, leaderboard: true }));
      try {
        ...
        setLoadingState((prev) => ({ ...prev, leaderboard: false }));
      }
    };

    if (user && token) {
      fetchAssignedEvents();
      // Fetch submissions and leaderboard for the first assigned event
      if (assignedEvents.length > 0) {
        fetchSubmissionsToReview(assignedEvents[0].id);
        fetchLeaderboard(assignedEvents[0].id);
      }
    }
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/judges/submissions/${selectedSubmission.id}/review`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${token}` },
        body: JSON.stringify({
          judgeId: user.id,
          ...selectedSubmission.scores,
          comments: "Comments from review form", // You'll need to add a state for comments
        }),
      });
  ...
  const renderEventsTab = () => (
    <div className="space-y-6">
      ...
      {loadingState.events ? (
        <div>Loading assigned events...</div>
      ) : (
        ...
        {selectedSubmission ? (
          <Card className="premium-card">
            ...
            <CardContent className="space-y-6">
              ...
              {/* Actions */}
              <div className="flex gap-2">
                <Button variant="hero" className="flex-1" onClick={handleSubmitReview}>
                  Submit Review
                </Button>
            ...
            leaderboard.map((entry) => {
              const getRankIcon = (rank: number) => {
                switch (rank) {
                  ...
                  default:
                    return (
                      <div className="w-5 h-5 flex items-center justify-center text-sm font-bold">{rank}</div>
                    );
                }
              };
